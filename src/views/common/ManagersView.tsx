...
interface Props {
  entity: any
  entityManagersStore: any
  entityManagerAddStore: any
  addEntityManagerAction: any
  clearEntityManagerAddAction: any
...
const ManagersView = (props: Props) => {
  const {
    entity,
    entityManagersStore,
    entityManagerAddStore,
    addEntityManagerAction,
    clearEntityManagerAddAction,
  ...
  const { loading, data: managers } = entityManagersStore

  const usersListStore = useSelector((state: any) => state.usersList)
  const { data: users } = usersListStore

  const { status: addMnanagerStatus } = entityManagerAddStore
  ...
  useEffect(() => {
    if (addMnanagerStatus) {
      ...
      dispatch(clearEntityManagerAddAction())
    }
  }, [addMnanagerStatus])

  useEffect(() => {
    if (entityId) dispatch(getEntityManagersAction(entityId))
  }, [entityId])

  const addNewManager = (newManager: any) => {
    if (newManager?.id?.length > 0) {
      dispatch(addEntityManagerAction(newManager?.id))
    }
  }
  ...
  return (
    <Grid container spacing={6}>
      ...
      <RemoveManagerDialog
        ...
        entity={entity}
        getEntityManagersAction={getEntityManagersAction}
        entityManagerRemoveStore={entityManagerRemoveStore}
        removeEntityManagerAction={removeEntityManagerAction}
        clearEntityManagerRemoveAction={clearEntityManagerRemoveAction}
      />
    </Grid>
  )
}
